# Hi, I'm Shreya K. R. 👋
**B.Tech (CSE, Data Science)** | Presidency University  
Aspiring MSc student (Germany 2026) — ML, Data Science, Applied AI, and Game Dev.

🔭 Currently: Finishing a capstone project & polishing GitHub portfolio  
🌱 Learning: German (A2) | Advanced ML topics | Reproducible research  
📫 Reach me: krshrey10 (GitHub) · [Email: your-email@example.com]

---

## Highlight Projects
- **PixelSaga** — Procedural world generation engine (game dev + deterministic noise systems).  
- **Mindshift** — Tactical rule-shifting card game (Unity/JS + rules engine).  
- **Stock_Price_Prediction_LSTM** — LSTM pipeline, experiment logbook & evaluation.  
- **Fake_News_Detection** — NLP pipeline, feature engineering & model benchmarking.

---

## Skills
Python · PyTorch/TensorFlow · NumPy · pandas · scikit-learn · NLP · LSTM · SQL · Docker · Git · Unity (basic) · Data viz

---

## What I value
Clean code, reproducible experiments, clear evaluation, and well-documented projects.

> Interested in collaborations or research internships — open to messaging!

